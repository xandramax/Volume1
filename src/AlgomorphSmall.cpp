...
json_t* AlgomorphSmall::dataToJson() {
    ...
    for (int scene = 0; scene < 3; scene++) {
        ...
        json_object_set_new(sceneForcedCarriersJ, "Forced Carriers", json_integer(forcedCarriers[scene].to_ullong()));
...
void AlgomorphSmall::dataFromJson(json_t* rootJ) {
    ...
    if (forcedCarriersJ) {
        ...
        json_array_foreach(forcedCarriersJ, sceneIndex, sceneForcedCarriers) {
            forcedCarriers[sceneIndex] = json_integer_value(json_object_get(sceneForcedCarriers, "Forced Carriers"));
        }
    }
